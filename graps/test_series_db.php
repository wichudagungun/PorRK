...
if(isset($_GET['week']) && $_GET['week']!=""){
	$more_q="AND DATE_FORMAT(TIMEIN,'%w')='".$_GET['week']."' ";
}

$q="
SELECT ROOM,TIMEIN,SUM(TIMEIN) as sum_TIMEIN FROM reportss
WHERE 1 $more_q
GROUP BY ROOM,DATE_FORMAT(TIMEIN,'%w-%m')
...
while($rs=mysql_fetch_array($qr)){
    ...
    $json_data[$rs['ROOM']][$char_month]=intval($rs['sum_TIMEIN']); // เก็บข้อมูลแต่ละเดือนถ้ามี
}
